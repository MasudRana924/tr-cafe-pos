  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <InventoryStat label="In Stock" value={ok.length} icon={CheckCircle2} color="success" bgColor="bg-green-50" borderColor="border-green-200" />
        <InventoryStat label="Low Stock" value={low.length} icon={AlertTriangle} color="warning" bgColor="bg-yellow-50" borderColor="border-yellow-200" />
        <InventoryStat label="Out of Stock" value={out.length} icon={PackageX} color="destructive" bgColor="bg-red-50" borderColor="border-red-200" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((p, i) => (
          <InventoryProductCard key={p.id} product={p} index={i} />
        ))}
      </div>
    </div>
  );
...
function InventoryProductCard({ product: p, index: i }: { product: Product; index: number }) {
  const status = p.quantity === 0 ? "out" : p.quantity <= 10 ? "low" : "ok";
  return (
    <div className="card-soft p-5 hover-lift animate-scale-in" style={{ animationDelay: `${i * 30}ms` }}>
      <div className="flex items-start justify-between mb-3">
        <div className="text-4xl">📦</div>
        {status !== "ok" && (
          <span
            className={`text-xs font-medium px-2.5 py-1 rounded-full ${status === "out" ? "bg-destructive/10 text-destructive" : "bg-warning/15 text-warning-foreground"}`}
          >
